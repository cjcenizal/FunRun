	...
	public class LoadObstaclesCommand extends Command {
		...
		override public function execute():void {
			...
			for ( var i:int = 0; i < len; i++ ) {
				...
				// Store this sucker.
				var data:ObstacleVO = makeObstacle( obstacle, material );
				obstaclesModel.addObstacle( new ObstacleData( data.mesh, data.boundingBoxes ) );
				if ( obstacle.flip ) {
					// Store mirror version if required.
					data = makeObstacle( obstacle, material, true );
					obstaclesModel.addObstacle( new ObstacleData( data.mesh, data.boundingBoxes ) );
				}
			}
		}
		
		private function makeObstacle( obstacle:ObstacleParser, material:MaterialBase, flip:Boolean = false ):ObstacleVO {
			...
			return new ObstacleVO( obstacleMesh, boundingBoxes );
		}
	}
}
...
class ObstacleVO {
	...
	public var boundingBoxes:Array;
	public function ObstacleVO( mesh:Mesh, boundingBoxes:Array ) {
		this.mesh = mesh;
